...
export const findAll = async (filters = {}) => {
  ...
  if (filters.statusId) q.where('tasks.statusId', filters.statusId);
  if (filters.executorId) q.where('tasks.executorId', filters.executorId);
  if (filters.createdBy) q.where('tasks.creatorId', filters.createdBy);

  const needLabelJoin = filters.labelId || filters.hasLabel;
  if (needLabelJoin) {
    q.leftJoin('tasks_labels', 'tasks.id', 'tasks_labels.taskId');
    if (filters.labelId) q.where('tasks_labels.labelId', filters.labelId);
    if (filters.hasLabel && !filters.labelId) q.whereNotNull('tasks_labels.labelId');
    q.groupBy('tasks.id');
  }
  ...
  const labelsRows = await knex('tasks_labels')
    .whereIn('taskId', ids)
    .join('labels', 'tasks_labels.labelId', 'labels.id')
    .select('tasks_labels.taskId', 'labels.id', 'labels.name');

  const labelsByTask = labelsRows.reduce((acc, label) => {
    const next = acc;
    next[label.taskId] = next[label.taskId] || [];
    next[label.taskId].push({ id: label.id, name: label.name });
    return next;
  }, {});

  return rows.map((row) => {
    const labels = labelsByTask[row.id] || [];
    return { ...row, labels, labelIds: labels.map((label) => label.id) };
  });
};
...
export const findById = async (id) => {
  const task = await knex('tasks').where({ 'tasks.id': id })
    ...
    .first();
  if (!task) return null;
  const labels = await knex('tasks_labels')
    .where({ taskId: id })
    .join('labels', 'tasks_labels.labelId', 'labels.id')
    .select('labels.id', 'labels.name');
  task.labels = labels;
  task.labelIds = labels.map((label) => label.id);
  return task;
};

export const create = async (attrs) => {
  const existing = await knex('tasks').where({ name: attrs.name }).first();
  if (existing) return findById(existing.id);
