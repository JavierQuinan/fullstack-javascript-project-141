...
describe('Tasks filtering', () => {
  ...
  it('filters by status, executor, label presence and creator', async () => {
    // create users
    const u1 = await userRepo.create({ firstName: 'A', lastName: 'One', email: 'a@example.com', password: 'x' });
    const u2 = await userRepo.create({ firstName: 'B', lastName: 'Two', email: 'b@example.com', password: 'x' });
    // statuses
    ...
    // tasks
    const t1 = await taskRepo.create({ name: 'T1', description: null, statusId: s1.id, creatorId: u1.id, executorId: u2.id, labelIds: [l1.id] });
    const t2 = await taskRepo.create({ name: 'T2', description: null, statusId: s2.id, creatorId: u2.id, executorId: null, labelIds: [] });
    const t3 = await taskRepo.create({ name: 'T3', description: null, statusId: s2.id, creatorId: u1.id, executorId: null, labelIds: [l2.id] });
    const t4 = await taskRepo.create({ name: 'T4', description: null, statusId: s1.id, creatorId: u1.id, executorId: null, labelIds: [] });
